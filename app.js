...
function setSyncStatus(message, isOnline = false) {
  if (!syncNotice) return;
  syncNotice.textContent = message;
  syncNotice.classList.toggle("is-online", isOnline);
}
...
async function showApp(vaultId) {
  ...
  currentUserLabel.textContent = cloudEnabled()
    ? `Підключення: ${shortVaultId(vaultId)}`
    : `Локально: ${shortVaultId(vaultId)}`;
  setSyncStatus(
    cloudEnabled()
      ? "Підключаю синхронізацію Firebase..."
      : "Синхронізація не увімкнена: Firebase-налаштування не знайдені.",
    false
  );
...
async function initCloudSync(vaultId) {
  ...
  try {
    ...
    setSyncStatus("Синхронізація Firebase увімкнена. Цей самий пароль відкриє ці записи на іншому пристрої.", true);
    ...
    setSyncStatus(
      "Firebase не підключився. Найчастіше причина у правилах Firestore: треба дозволити read/write для birthdayVaults.",
      false
    );
    setAuthMessage("");
